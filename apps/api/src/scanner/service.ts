...
export class ScannerService {
  ...
  public async isEnabled(): Promise<boolean> {
    const setting = await db.query.appSettings.findFirst({ where: eq(appSettings.key, SCANNER_ENABLED_SETTING) });
    return setting ? setting.value === 'true' : true;
  }

  public async setEnabled(enabled: boolean): Promise<boolean> {
    await db.insert(appSettings).values({ key: SCANNER_ENABLED_SETTING, value: String(enabled), updatedAt: timestamp() })
  ...
  public async isAutoJoinEnabled(): Promise<boolean> {
    const setting = await db.query.appSettings.findFirst({ where: eq(appSettings.key, AUTO_JOIN_ENABLED_SETTING) });
    return setting?.value === 'true';
  }

  public async setAutoJoinEnabled(enabled: boolean): Promise<boolean> {
    await db.insert(appSettings).values({ key: AUTO_JOIN_ENABLED_SETTING, value: String(enabled), updatedAt: timestamp() })
      .onConflictDoUpdate({ target: appSettings.key, set: { value: String(enabled), updatedAt: timestamp() } });
    return enabled;
  }
  ...
  public async processIncomingMessage(message: WAMessage): Promise<number> {
    ...
    const group = await db.query.groups.findFirst({ where: eq(groups.whatsappGroupJid, groupJid) });
  ...
  public async recordSighting(link: InviteLink, sighting: { groupJid: string; groupName: string; messageId: string | null; occurredAt?: string }): Promise<{ id: string; isNew: boolean }> {
    ...
    const existing = await db.query.discoveredLinks.findFirst({ where: eq(discoveredLinks.inviteUrl, link.inviteUrl) });
    const result = await db.insert(discoveredLinks).values({
      id: randomUUID(), inviteUrl: link.inviteUrl, inviteCode: link.inviteCode,
      ...
      target: discoveredLinks.inviteUrl,
  ...
  public async updateLink(id: string, changes: { status?: LinkStatus; notes?: string | null }) {
    ...
    const updated = await db.update(discoveredLinks).set(update).where(eq(discoveredLinks.id, id)).returning();
  ...
  public async getLink(id: string) {
    const [link] = await db.select({ id: discoveredLinks.id, inviteCode: discoveredLinks.inviteCode, inviteUrl: discoveredLinks.inviteUrl })
      .from(discoveredLinks).where(eq(discoveredLinks.id, id)).limit(1);
    return link ?? null;
  }

  public async deleteLink(id: string): Promise<void> { await db.delete(discoveredLinks).where(eq(discoveredLinks.id, id)); }
  ...
  public async exportLinks() {
    ...
    }).from(discoveredLinks).orderBy(asc(discoveredLinks.firstSeenAt));
  }
}
